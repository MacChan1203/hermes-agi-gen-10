...
from .config import (
    ...
    GOAL_QUEUE_COMPOSITE_PRIORITY_W,
    GOAL_QUEUE_COMPOSITE_VALUE_W,
    GOAL_QUEUE_COMPOSITE_DIFFICULTY_W,
)
...
def _escape_for_prompt(text: str) -> str:
    """外部データをLLMプロンプトに埋め込む前にサニタイズする。

    - 制御的な指示パターン (system/assistant ロールの偽装) を無効化
    - 区切り文字の注入を防止
    - 長すぎる入力を切り詰め
    """
    if not isinstance(text, str):
        text = str(text)
    # Unicode正規化 (NFKC) — 全角文字 (ｓｙｓｔｅｍ等) を半角に統一してから検査
    text = unicodedata.normalize("NFKC", text)
    # プロンプト区切り文字・ロール偽装パターンを除去
    text = re.sub(r"(?i)(system|assistant|user)\s*:", "[role]:", text)
    # 三重バッククォートやフェンスを無効化 (プロンプト構造の破壊防止)
    text = text.replace("```", "'''")
    # === や --- などの区切り線注入を無効化
    text = re.sub(r"={3,}", "==", text)
    text = re.sub(r"-{3,}", "--", text)
    # 過度に長い入力を切り詰め (プロンプト膨張攻撃の防止)
    max_len = 2000
    if len(text) > max_len:
...
class QueuedGoal:
    ...
    def composite_score(self) -> float:
        """優先度・価値・難易度を総合したスコア。"""
        return (
            (self.priority_score * GOAL_QUEUE_COMPOSITE_PRIORITY_W)
            + (self.estimated_value * GOAL_QUEUE_COMPOSITE_VALUE_W)
            + ((1 - self.estimated_difficulty) * GOAL_QUEUE_COMPOSITE_DIFFICULTY_W)
        )
...
class MetaCognition:
    ...
    STUCK_FAILURE_THRESHOLD = STUCK_FAILURE_THRESHOLD
    ...
    def suggest_pivot(self, state: AgentState, memory: LongTermMemory) -> Optional[str]:
        ...
        error_history = state.working_memory.get("error_history", [])
        if error_history:
            top_error = _escape_for_prompt(Counter(error_history).most_common(1)[0][0])
            pivot_map = {
    ...
    def _llm_generate_goals(self, state: AgentState, memory: LongTermMemory) -> None:
        ...
        prompt = _GOAL_GENERATION_PROMPT.format(
            completed_goal=_escape_for_prompt(state.user_goal),
            completed_steps=_escape_for_prompt(", ".join(state.completed_steps[-5:]) or "なし"),
            failed_steps=_escape_for_prompt(", ".join(state.failed_steps[-3:]) or "なし"),
            observations=_escape_for_prompt("; ".join(state.observations[-3:]) or "なし"),
            learned_facts=_escape_for_prompt("; ".join(learned_facts[-3:]) or "なし"),
                ...
                for item in data:
                    if isinstance(item, dict) and item.get("goal"):
                        try:
                            ...
                            continue  # 不正なLLM応答は個別にスキップ
        except Exception:
            logger.debug("LLMゴール生成に失敗", exc_info=True)
    ...
    def _generate_curiosity_goals(self, state: AgentState, memory: LongTermMemory) -> None:
        ...
        prompt = _CURIOSITY_PROMPT.format(
            known_facts=_escape_for_prompt(facts_text),
            failure_patterns=_escape_for_prompt(failures_text),
            world_model_summary=_escape_for_prompt(wm_summary),
        )
        ...
        try:
            ...
            logger.debug("好奇心駆動ゴール生成に失敗", exc_info=True)
