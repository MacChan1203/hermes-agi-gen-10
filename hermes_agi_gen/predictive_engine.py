...
class PredictiveEngine:
    ...
    def predict(self, action: str, goal: str = "", context: str = "") -> Prediction:
        ...
        success_probability = self._calculate_success_probability(
            failure_risk, success_evidence, confidence
        )
    ...
    def record_outcome(
        ...
        if predicted_success == actual_success:
            ...
            # 方向性が間違っていた（大きな誤差）
            error = 0.5 + abs(prediction.success_probability - (1.0 if actual_success else 0.0)) * 0.5
    ...
    def _assess_failure_risk(self, action: str) -> float:
        ...
        try:
            ...
            for failure in failures:
                ...
                if pattern and len(pattern) > 3 and pattern in action_lower:
    ...
    def _calculate_confidence(
        ...
        """予測確信度を計算する。"""
        # データが多いほど確信度が上がる
        history_factor = min(1.0, len(self._prediction_history) / 50)
        base_confidence = 0.3 + history_factor * 0.4
    ...
    def _calculate_success_probability(
        ...
        # 失敗リスクと成功根拠のバランス
        base = 0.5
        base += (success_evidence - failure_risk) * 0.4
